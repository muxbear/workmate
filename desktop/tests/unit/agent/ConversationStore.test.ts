...
function makeCheckpointer(tuples: CheckpointTuple[]): {
  ...
  return {
    ...
    getTuple: vi.fn(async (config: { configurable: { thread_id: string } }) => {
      return (
        tuples.find((t) => t.config.configurable?.thread_id === config.configurable.thread_id) ??
        undefined
      )
    }),
...
describe('ConversationStore（基于 LangGraph checkpointer 的会话服务）', () => {
  ...
  it('saveAutoTitle 写入且不覆盖用户手动重命名', async () => {
    ...
    const store = new ConversationStore(
      () => makeCheckpointer(tuples) as never,
      () => ds.getDb()
    )
  ...
  it('bindWorkspace 落库后 listConversations 从业务表读绑定（checkpoint metadata 无 workspace 也可）', async () => {
    ...
    const list = await store.listConversations('u1')
    expect(list[0].workspace).toEqual({
      id: 'ws-2',
      name: '我的工作空间',
      dir: '/tmp/我的工作空间'
    })
    ...
    const store2 = new ConversationStore(
      () => makeCheckpointer([boundTuple]) as never,
      () => ds.getDb()
    )
...
describe('getMessages 按轮折叠（多内部 AIMessage/ToolMessage → 一条 AI 展示消息）', () => {
  it('tool/system 不渲染，一轮多条 AI 合并 content/reasoning', async () => {
    const tuples = [
      makeTuple(
        'u:u1:c1',
        [
          { id: 'sys-1', role: 'system', content: '系统提示' },
          { id: 'h1', role: 'human', content: '帮我写文件' },
          {
            id: 'a1',
            role: 'ai',
            content: [
              { type: 'reasoning', reasoning: '思考一' },
              { type: 'tool_call', id: 'c1', name: 'write_file', args: {} }
            ]
          },
          { id: 't1', role: 'tool', content: '写入成功' },
          {
            id: 'a2',
            role: 'ai',
            content: [
              { type: 'text', text: '已生成：' },
              { type: 'text', text: '方案.md' }
            ]
          }
        ],
        new Date(100)
    ...
    expect(messages[0]).toEqual({ id: 'h1', role: 'user', content: '帮我写文件' })
  ...
  it('AI 消息先于 user 消息（异常形态）时按独立轮次展示', async () => {
    const tuples = [
      makeTuple(
        'u:u1:c1',
        [
          { id: 'a0', role: 'ai', content: '主动开场' },
          { id: 'h1', role: 'human', content: '追问' },
          { id: 'a1', role: 'ai', content: '回答' }
        ],
        new Date(100)
      )
    ]
    ...
    expect(messages[0].content).toBe('主动开场')
  })
})
