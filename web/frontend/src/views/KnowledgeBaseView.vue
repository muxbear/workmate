...
const activeGroup = computed(() =>
  store.activeNav === 'overview'
    ? null
    : KB_GROUPS.find((g) => g.id === store.activeNav) ?? null,
)

const shareManageKb = computed<KB | null>(() => {
  const id = shareManageKbId.value
  if (!id) return null
  const fromGroup = store.groups.sharedByMe?.items.find((k) => k.id === id)
  if (fromGroup) return fromGroup
  return store.selectedKb?.id === id ? store.selectedKb : null
})
...
function handleDeleteConfirm(kb: KB) {
...
function openShareManage(kbId: string) {
  shareManageKbId.value = kbId
}

async function handleCancelShare(kbId: string) {
  try {
    await store.cancelShares(kbId)
    ...
    <div class="panel-right">
      ...
      <div v-else class="panel-right-inner">
        ...
        <KbGroupList
          v-if="activeGroup"
          :key="activeGroup.id"
          :scope="activeGroup.id as Exclude<KbScope, 'overview'>"
        />
